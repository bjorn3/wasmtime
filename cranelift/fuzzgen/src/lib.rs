    ...
    'data: 'r,
{
    ...
    fn run_func_passes(&mut self, func: Function, isa_without_verifier: &dyn TargetIsa) -> Result<Function> {
        ...
        ctx.canonicalize_nans(isa_without_verifier.as_ref())
    ...
    pub fn generate_func(
        ...
        name: UserFuncName,
        isa: OwnedTargetIsa,
        isa_without_verifier: &dyn TargetIsa,
        ...
        self.run_func_passes(func, &*isa_without_verifier)
    }

    /// Generate a random set of cranelift flags.
    /// Only semantics preserving flags are considered
    pub fn generate_flags(&mut self, target_arch: Architecture, enable_verifier: bool) -> arbitrary::Result<Flags> {
