...
impl Inst {
    ...
    pub(crate) fn invoke_known(default: MachLabel, info: Box<CallInfo<ExternalName>>) -> Inst {
        Inst::InvokeKnown { info, default }
    }

    pub(crate) fn invoke_unknown(default: MachLabel, info: Box<CallInfo<RegMem>>) -> Inst {
        info.dest.assert_regclass_is(RegClass::Int);
        Inst::InvokeUnknown { info, default }
    }
