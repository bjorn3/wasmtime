...
use target_lexicon::{Architecture, Riscv32Architecture, Riscv64Architecture, Triple};
...
impl RiscvBackend {
    ...
    pub fn new_with_flags(triple: Triple, flags: settings::Flags) -> RiscvBackend {
    ...
    fn compile_vcode(
        ...
        let abi = Box::new(abi::RiscvABICallee::new(func, self.flags().clone())?);
        compile::compile::<RiscvBackend>(func, self, abi, emit_info)
    }
}

impl MachBackend for RiscvBackend {
    fn compile_function(
        &self,
        func: &Function,
        want_disasm: bool,
    ...
    fn emit_unwind_info(
        ...
        Ok(match (result.unwind_info.as_ref(), kind) {
            ...
            (Some(_info), UnwindInfoKind::Windows) => {
                // TODO: support Windows unwind info on RISC-V
                None
            }
            _ => None,
        })
...
pub fn isa_builder(triple: Triple) -> IsaBuilder {
    ...
    IsaBuilder {
        ...
        constructor: |triple, shared_flags, _| {
            let backend = RiscvBackend::new_with_flags(triple, shared_flags);
...
mod test {
    ...
    fn test_compile_function() {
        ...
        let backend = RiscvBackend::new_with_flags(
