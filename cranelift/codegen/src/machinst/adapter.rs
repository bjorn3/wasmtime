...
impl TargetIsa for TargetIsaAdapter {
    ...
    fn legalize_signature(&self, sig: &mut Cow<ir::Signature>, _current: bool) {
        ...
        fn legalize_args(args: &[AbiParam]) -> Vec<AbiParam> {
            args.iter().cloned().flat_map(|param| {
        ...
        if sig.params.iter().any(|param| param.value_type == types::I128) {
            sig.to_mut().params = legalize_args(&sig.params);
        }
        if sig.returns.iter().any(|param| param.value_type == types::I128) {
